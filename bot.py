...
def fetch_messages(channel):
    ...
    for block in soup.select("div.tgme_widget_message"):
        ...
        # Convert actual <br> line breaks to real newlines FIRST, then
        # extract text with no artificial separator. Using get_text("\n",...)
        # directly inserts a newline between the text of EVERY separate tag
        # BeautifulSoup walks through (not just real line breaks) — so a
        # message that opens with several hashtags/mentions/bold words each
        # wrapped in their own <a>/<span> tag would come out looking like
        # one word per line. Replacing <br> tags explicitly avoids that.
        if text_div:
